...
# Patch pour compatibilité avec la nouvelle version d’icalendar
if not hasattr(icalendar.timezone, 'equivalent_timezone_ids_result'):
    def equivalent_timezone_ids_result(tzid):
        """
        Fonction de remplacement pour equivalent_timezone_ids_result.
        Retourne simplement le tzid dans une liste.
        """
        return [tzid]
    icalendar.timezone.equivalent_timezone_ids_result = equivalent_timezone_ids_result
    _LOGGER.debug("Patched icalendar.timezone.equivalent_timezone_ids_result")
...
# Liste des plateformes supportées (pour cet exemple, uniquement sensor)
PLATFORMS = ["sensor"]
...
class ICalEvents:
    ...
    async def update(self):
        ...
        if text is not None:
            # Certains calendriers contiennent des NULL-bytes qu'il faut retirer
